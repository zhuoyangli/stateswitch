"""
Plot cingulate Schaefer 400 (17-network) parcels on the inflated surface.

Groups shown (medial + lateral views, both hemispheres):
  - ContA_Cingm   (mid cingulate)
  - ContC_Cingp   (posterior cingulate)

Output:
  figs/schaefer_rois/acc_parcels_surface.png
"""
...
# Match: hemi_prefix in label AND substr in label
GROUPS = [
    ('Cont Cingm', 'Cingm'),    # mid cingulate
    ('Cont Cingp', 'Cingp'),    # posterior cingulate
]

GROUP_COLORS = ['#377eb8', '#4daf4a']  # blue, green


def load_acc_texture(annot_path, hemi_prefix):
...
def main():
    ...
    tex_lh, info_lh, n_lh = load_acc_texture(LH_ANNOT, 'LH')
    tex_rh, info_rh, n_rh = load_acc_texture(RH_ANNOT, 'RH')
    ...
    #   Row 0: medial views   (LH, RH)  — ACC is primarily medial
    #   Row 1: lateral views  (LH, RH)
    # -------------------------------------------------------------------------
    fig, axes = plt.subplots(2, 2, figsize=(14, 10),
                             subplot_kw={'projection': '3d'})
    fig.suptitle('Schaefer 400 (17-network) — Cingulate Parcels',
    ...
    out_path = out_dir / 'acc_parcels_surface.png'
