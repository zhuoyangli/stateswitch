"""
svf_boundary_wspc.py — SVF Trial-Offset Within-Subject Pattern Correlation
...
Computes within-subject pattern correlation across all N_CATEGORIES × N_WINDOWS
conditions. Shared categories are discovered at runtime from psychopy CSVs.

Differs from svf_boundary_ispc.py only in the correlation step:
...
Usage:
    uv run python srcs/fmrianalysis/svf_boundary_wspc.py
    uv run python srcs/fmrianalysis/svf_boundary_wspc.py --roi pmc ag --n_jobs 4
    uv run python srcs/fmrianalysis/svf_boundary_wspc.py --hp --no-cache-wspc
    uv run python srcs/fmrianalysis/svf_boundary_wspc.py --min-subjects 6
"""
...
RESULT_CACHE_DIR = ANALYSIS_CACHE_DIR / 'svf_boundary_wspc'
OUTPUT_DIR       = FIGS_DIR / 'svf_boundary_wspc' / 'default'
...
def make_combined_figure(wspc_by_roi, vmax=0.3, horizontal=False):
    ...
    if horizontal:
        ...
        fig.suptitle(
            'WSPC between mean activation patterns of peri-boundary windows in each SVF category',
        ...
        fig.suptitle('SVF Boundary WSPC — All ROIs', fontsize=13, fontweight='bold')
        plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description='SVF boundary WSPC.')
    ...
    OUTPUT_DIR = FIGS_DIR / 'svf_boundary_wspc' / f'{preset_name}{hp_suffix}'
