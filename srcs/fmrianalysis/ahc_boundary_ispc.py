"""
ahc_boundary_ispc.py — AHC Trial-Offset Inter-Subject Pattern Correlation
...
"next"). Computes LOO inter-subject pattern correlation across all
...
Usage:
    uv run python srcs/fmrianalysis/ahc_boundary_ispc.py
    uv run python srcs/fmrianalysis/ahc_boundary_ispc.py --roi pmc ag --n_jobs 4
    uv run python srcs/fmrianalysis/ahc_boundary_ispc.py --hp --no-cache-ispc
    uv run python srcs/fmrianalysis/ahc_boundary_ispc.py --min-subjects 6
"""
...
RESULT_CACHE_DIR  = ANALYSIS_CACHE_DIR / 'ahc_boundary_ispc'
OUTPUT_DIR        = FIGS_DIR / 'ahc_boundary_ispc' / 'default'
...
def get_ahc_offsets_by_prompt(subject, session):
    ...
    if len(rows) < 2 or len(rows) != len(prompts):
        # Mismatch — skip to avoid misalignment
...
def compute_group_ispc(all_patterns):
    ...
    Uses NaN-aware LOO means so that subjects missing a prompt do not
...
def make_combined_figure(ispc_by_roi, vmax=0.3, horizontal=False):
    ...
    if horizontal:
        ...
        fig.suptitle(
            'ISPC between mean activation patterns of peri-boundary windows in each AHC prompt',
        ...
        fig.suptitle('AHC Boundary ISPC — All ROIs', fontsize=13, fontweight='bold')
        plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description='AHC boundary ISPC.')
    ...
    OUTPUT_DIR = FIGS_DIR / 'ahc_boundary_ispc' / f'{preset_name}{hp_suffix}'
