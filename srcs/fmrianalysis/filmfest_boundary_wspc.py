"""
filmfest_boundary_wspc.py — Filmfest Within-Subject Pattern Correlation

For each of 8 movie-onset boundaries (movies 2–5 and 7–10; first movie of each
run excluded), extracts time-window activation patterns per ROI. Computes
pairwise within-subject pattern correlation across all conditions
(8 movies × N_windows), producing a matrix per ROI.

Differs from filmfest_boundary_ispc.py only in the correlation step:
...
Level-1 cache (shared with filmfest_boundary_ispc.py):
  ANALYSIS_CACHE_DIR/roi_voxels/{sub}_{ses}_task-{task}_roi-{roi}_sm6.npz

Usage:
    uv run python srcs/fmrianalysis/filmfest_boundary_wspc.py
    uv run python srcs/fmrianalysis/filmfest_boundary_wspc.py --hp
    uv run python srcs/fmrianalysis/filmfest_boundary_wspc.py --post4 --hp
    uv run python srcs/fmrianalysis/filmfest_boundary_wspc.py --no-cache
    uv run python srcs/fmrianalysis/filmfest_boundary_wspc.py --n_jobs 4
    uv run python srcs/fmrianalysis/filmfest_boundary_wspc.py --roi pmc ag eac
"""
...
OUTPUT_DIR      = FIGS_DIR / 'filmfest_wspc' / 'default'  # reassigned in main
RESULT_CACHE_DIR = ANALYSIS_CACHE_DIR / 'filmfest_wspc'
...
def apply_window_preset(name):
    """Update module-level window globals to the named preset ('default', 'hrf', or 'post4')."""
...
def extract_boundary_patterns(voxel_data, boundary_trs):
    """Extract time-window patterns around each boundary.
    ...
    for btr in boundary_trs:
        ...
        if ok:
            valid_patterns.append(np.stack(win_patterns, axis=0))  # (N_WINDOWS, V)
...
def get_subject_patterns(subject, roi_key, parcel_ids, do_hp=False, force_voxels=False):
    ...
    Returns
    -------
    patterns : (8, N_WINDOWS, V) or None if data unavailable
    valid_mask : (8,) bool array — which of the 8 boundaries were valid
    """
...
def flatten_patterns(patterns):
    """Reshape (8, N_WINDOWS, V) → (N_CONDITIONS, V) in movies-nested-in-windows order.

    Output order: [M2_W0..M10_W0, M2_W1..M10_W1, ..., M2_WN..M10_WN]
...
def make_wspc_figure(wspc_matrix, roi_name, vmax=0.3):
    """Plot N_CONDITIONS×N_CONDITIONS WSPC heatmap with block structure.
    ...
    # ---- Window block labels on the y-axis ----
...
def make_combined_figure(wspc_by_roi, vmax=0.3, horizontal=False):
    ...
    if horizontal:
        ...
        fig.suptitle(
            'WSPC between mean activation patterns of peri-boundary windows in each movie boundary',
        ...
        fig.suptitle('Filmfest WSPC — All ROIs', fontsize=13, fontweight='bold')
        plt.tight_layout()
...
def run_roi(roi_key, roi_name, parcel_ids, subjects, do_hp=False,
    ...
    """Full pipeline for one ROI: extract → compute WSPC → save figure.

    Returns group WSPC (N_CONDITIONS, N_CONDITIONS) array.
    """
    ...
    if not force_wspc and wspc_cache.exists():
        print(f"  [{roi_key}] Loading WSPC from cache ...")
        data = np.load(wspc_cache, allow_pickle=True)
        group_wspc = data['group_wspc']
    else:
        print(f"\n{'='*60}")
        ...
        for subj in subjects:
            ...
            if pats is None:
                ...
                continue
            flat = flatten_patterns(pats)  # (N_CONDITIONS, V)
        ...
        all_patterns = np.stack(subject_patterns, axis=0)  # (N_subj, N_CONDITIONS, V)
        print(f"  [{roi_key}] Computing group WSPC ({all_patterns.shape[0]} subjects)...")
        group_wspc, per_subj = compute_group_wspc(all_patterns)

        np.savez_compressed(
            wspc_cache,
            group_wspc=group_wspc,
...
def main():
    parser = argparse.ArgumentParser(
        description='Filmfest within-subject pattern correlation (N_CONDITIONS×N_CONDITIONS WSPC).'
    )
    ...
    OUTPUT_DIR = FIGS_DIR / 'filmfest_wspc' / f'{preset_name}{hp_suffix}'

    subjects = sorted(FILMFEST_SUBJECTS.keys())
    print(f"Subjects: {subjects}")
    print(f"ROIs: {args.roi}")
    print(f"Window preset: {preset_name} ({N_CONDITIONS} conditions)")
    ...
    for roi_key, roi_name, parcel_ids in roi_spec_filtered:
        mat = run_roi(
            ...
            force_wspc=(args.no_cache or args.no_cache_wspc),
            vmax=args.vmax,
        )
        wspc_by_roi[roi_key] = mat

    # Combined figure if more than one ROI
    if sum(v is not None for v in wspc_by_roi.values()) > 1:
        combined_path = OUTPUT_DIR / 'all_rois_wspc.png'
